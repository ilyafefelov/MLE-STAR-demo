...
### Updated (n=20) findings
- We ran `digits` and `iris` with `n_runs=20` to improve statistical power and reduce variance. For `digits`, `minimal` and `no_feature_engineering` now outperform the `full` pipeline (mean minimal ~0.9396, full ~0.9194; ANOVA p ≈ 5.99e-07). For `iris`, `minimal` and `no_feature_engineering` also outperform the `full` pipeline (mean minimal ~0.9567, full ~0.9117; ANOVA p ≈ 6.78e-07). These runs reinforce that for some datasets simpler pipelines may generalize better than tuned/ensembled pipelines generated by Gemini in our harness.
